...
class ReporteDiario(models.AbstractModel):
    ...
    def lineas(self, datos):
        ...
        if datos['agrupado_por_dia']:
            ...
            self.env.cr.execute('select a.id, a.code as codigo, a.name as cuenta, t.include_initial_balance as balance_inicial, l.debit as debe, l.credit as haber, ' \
			    #' l.invoice_id as numero_documento, l.internal_note as comentario_documento, l.name as nombre_documento, l.move_id as numero_movimiento, '\
                ' l.invoice_id as numero_documento, l.name as nombre_documento, l.move_id as numero_movimiento, '\
				' l.date as fecha_movimiento '\
            	'from account_move_line l join account_account a on(l.account_id = a.id)' \
            	'join account_account_type t on (t.id = a.user_type_id)' \
            	'where a.id in ('+accounts_str+') and l.date >= %s and l.date <= %s ORDER BY a.id',
            (datos['fecha_desde'], datos['fecha_hasta']))
            ...
            for r in self.env.cr.dictfetchall():
                ...
                linea = {
					...
					'numero_movimiento': r['numero_movimiento'],
