...
class AccountMove(models.Model):
    ...
    def update_dates_invoices(self):
        account_moves = self.env['account.move'].search([
            ("name", 'like', 'RETENCION')
        ])
        print('moves', account_moves)
        for move in account_moves:
            for line in move.line_ids:
                if line.date != line.invoice_id.date_invoice:
                    print(line.invoice_id.number, line.date, line.invoice_id.date_invoice, line.move_id.date)

                    sql = "UPDATE account_move_line"
                    sql += " SET date = %s"
                    sql += " WHERE id = %s"

                    params = [line.invoice_id.date_invoice, line.id]
                    ...
                    params = [line.invoice_id.date_invoice, line.move_id.id]
...
class AccountPayment(models.Model):
    ...
    def _get_move_vals(self, journal=None):
        ...
        for payment in self:
            print('MOVE VALS', move_vals, payment)
            if payment.bank_operation_ref:
                move_vals['bank_operation_ref'] = payment.bank_operation_ref
