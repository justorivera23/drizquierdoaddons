...
class AccountInvoice(models.Model):
    ...
    def get_fel_certifier(self):
        for rec in self:
            ...
            if fel_certifier == 'digifact':
                rec.infile_fel_active = False
            rec.fel_certifier = fel_certifier
    ...
    def action_invoice_open(self):
        ...
        print('Type Inv', self.type, self.fel_gt_invoice_type)
    ...
    def set_data_for_invoice(self, fel_certifier):
        ...
        if fel_certifier == 'infile':
            establishment_code = self.company_id.infile_establishment_code
        if fel_certifier == 'digifact':
            if self.journal_id:
                establishment_code = self.journal_id.digifact_establishment_code
            else:
                establishment_code = self.company_id.digifact_establishment_code
        ...
        xml_emisor = ET.SubElement(xml_data_emision, "{" + xmlns + "}Emisor", AfiliacionIVA="GEN", CodigoEstablecimiento=establishment_code, CorreoEmisor=self.company_id.email, NITEmisor=self.company_id.vat, NombreComercial=self.company_id.name, NombreEmisor=self.company_id.company_registry)
        ...
        if self.company_id.fel_company_type != "1" or self.company_id.fel_company_code != "1":
            ET.SubElement(xml_frases, "{" + xmlns + "}Frase", TipoFrase="1", CodigoEscenario="1")
        ...
        for line in invoice_line:
            ...
            if line.invoice_line_tax_ids:
                ...
                for tax in line.invoice_line_tax_ids:
                    ...
                    print('TAX', price_tax, base, line.price_unit, line.quantity)
    ...
    def set_data_for_invoice_cambiaria(self, fel_certifier):
        ...
        # Frases
        xml_frases = ET.SubElement(xml_data_emision, "{" + xmlns + "}Frases")
        if self.company_id.fel_company_type != 1 and self.company_id.fel_company_code != 1:
            ET.SubElement(xml_frases, "{" + xmlns + "}Frase", TipoFrase="1", CodigoEscenario="1")
        ET.SubElement(xml_frases, "{" + xmlns + "}Frase", TipoFrase=self.company_id.fel_company_type, CodigoEscenario=self.company_id.fel_company_code)
    ...
    def send_data_api(self, xml_data=None, type_document='fel'):
        ...
        if fel_certifier == 'digifact':
            ...
            if type_document == 'fel':
                ...
                if 'Codigo' in json_certificate_response:
                    if json_certificate_response['Codigo'] == 1:
                        print('RESPUESTA', json_certificate_response)
                        if 'AcuseReciboSAT' in json_certificate_response:
                            sat_ref_id = json_certificate_response['AcuseReciboSAT']
...
def store_sent_xml(self, xml_content, vat, date_due, certifier="digifact"):
    ...
    if certifier == "digifact":
        ...
        sql = "INSERT INTO pt_multicert_felgt_digifact_xml_sent(name, xml_content, api_sent, create_uid, create_date, write_uid, write_date)"
